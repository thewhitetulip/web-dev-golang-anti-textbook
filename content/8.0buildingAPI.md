...
We are using `jwt-go` version 3. We have to define a custom claims struct which we will use in token generation and token verification. The StandardClaims contains the exirty data and who issued it.
