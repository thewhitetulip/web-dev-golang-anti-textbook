...
doesn't give built in libraries to interface with different database vendors, but they have defined an interface at `database/sql`. If you use
database drivers which follow this interface then one can switch underlying database at minimal change.  
