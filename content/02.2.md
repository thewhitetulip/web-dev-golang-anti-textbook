...
We use the keyword var to define a variable. Note that in Go the variable type comes **after** the variable name.
...
`_` variable is called the blank variable and it is used to ignore a value. This is a useless example, but we'll see its example soon enough.

	_, b := 34, 35 //use the blank operator to throw away a value
