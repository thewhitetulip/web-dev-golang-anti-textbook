...
This is because you can't use `{{.Navigation}}` inside the range block because if we use `{{.Navigation}}` in a range block it'll expect that we have 
a Navigation field in the Categories object, so we need to create a variable and then use it to compare if the category names are equal or not.
...
- Previous section: [Uploading Files](content/2.5UploadingFiles.md)
- Next section: [Working with forms](content/4.0authentication.md)